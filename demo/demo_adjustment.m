...
for k = 2:Nsteps 
    % ---- 状态自由传播 ---- 
    w = [q_WFM*randn(); q_RWFM*randn()]; 
    X = F*x + w; 
    
    % ---- PLL 控制误差（使用 offset） ---- 
    e1 = OFFSET_CONTROL(X(1), delta_th, H_max); 
    %e1 = -0.1 * X(1); % 直接使用 offset 作为误差
    X(1) = X(1) + e1; % offset 控制作用于 offset 
    
    % ---- 三阶 PLL 更新 (使用 OMEGA_CONTROL 函数) ---- 
    % [u1, z1, z2, z3, y1, y2] = OMEGA_CONTROL(e1, X(2), z1, z2, z3, y1, y2, K1, K2, K3, G1, G2, omega_th); 
